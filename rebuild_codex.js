...
async function main() {
    ...
    console.log("Running npm install with electron environment (prebuilt-first)...");
    let nativeInstallSucceeded = false;
    try {
        // Allow package bin links so `prebuild-install` can run on older macOS toolchains.
        ...
        nativeInstallSucceeded = true;
    } catch (e) {
        console.warn("Prebuilt install failed. Retrying native build from source...");
        try {
            execSync(`npm install --force`, {
                cwd: tempBuildDir,
                env: {
                    ...
                    // Force C++20 for Electron 40 compatibility on newer toolchains.
            ...
            if (os.platform() === 'darwin' && os.release().startsWith('21.')) {
                console.warn("Detected macOS Monterey (Darwin 21). Source builds may fail with older Command Line Tools; prebuilt binaries are required.");
            }
        }
    }

    if (!nativeInstallSucceeded) {
        console.warn("Native modules may still be arm64 and the app may fail to launch on Intel.");
    }
...
# Finder/launched apps on macOS often get a minimal PATH, which hides npm globals
# (gemini, openclaw, pnpm, etc.) installed under nvm/Homebrew.
for d in "$HOME/Library/pnpm" "/usr/local/bin" "/opt/homebrew/bin" "$HOME/.nvm/versions/node"/*/bin; do
  if [ -d "$d" ]; then
    PATH="$d:$PATH"
  fi
done
...
`;
    ...
    if (sourceCodexBin) {
        ...
        const userNodeGlobalShimPath = path.join(binDir, 'user-node-global');
        const userNodeGlobalShim = `#!/bin/bash
# Bridge commands from the Codex app's minimal PATH to user-installed npm globals.
set -e
...
find_target() {
  ...
  for c in "/usr/local/bin/$NAME" "/opt/homebrew/bin/$NAME" "$HOME/Library/pnpm/$NAME"; do
    if [ -x "$c" ]; then
      printf '%s\\n' "$c"
      return 0
    fi
...
if [ -z "$TARGET" ]; then
  echo "$NAME: not found (expected in nvm/Homebrew/pnpm global bins)" >&2
  exit 127
fi

TARGET_DIR="$(cd "$(dirname "$TARGET")" && pwd)"
export PATH="$TARGET_DIR:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin:$HOME/Library/pnpm:$PATH"

exec "$TARGET" "$@"
`;
        fs.writeFileSync(userNodeGlobalShimPath, userNodeGlobalShim);
        fs.chmodSync(userNodeGlobalShimPath, '755');
        for (const shimName of ['gemini', 'kilo', 'kilocode']) {
            const shimPath = path.join(binDir, shimName);
            try {
                if (fs.existsSync(shimPath)) {
                    fs.rmSync(shimPath, { force: true });
                }
                fs.symlinkSync('user-node-global', shimPath);
            } catch (e) {
                console.warn(`Failed to create shim for ${shimName}: ${e.message}`);
            }
        }
        ...
        console.warn(`WARNING: Could not find local x64 Codex binary in ${CODEX_CLI_PATH}`);
    }
